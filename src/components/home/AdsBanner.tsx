...
export default function AdsBanner({ ads }: Props) {
  ...
  return (
    <section className="relative overflow-hidden py-4 bg-slate-50 dark:bg-[#111111] border-y border-slate-200 dark:border-[#2a2a2a]">
      <div className="flex animate-marquee gap-4 w-max">
        {items.map((ad, i) => (
          <div
            key={`${ad.id}-${i}`}
            className="flex items-center gap-3 bg-white dark:bg-[#1a1a1a] border border-slate-200 dark:border-[#2a2a2a] rounded-xl px-5 py-3 min-w-max shadow-sm dark:shadow-none"
          >
            <span
